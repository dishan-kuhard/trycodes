...
function filterRecipes(recipes, filter) {
    return recipes.filter(recipe => filter === 'all' || recipe.difficulty === filter);
}

// Function to sort recipes by name
function sortByName(recipes, order = 'asc') {
...
function sortByTime(recipes, order = 'asc') {
    return [...recipes].sort((a, b) => order === 'asc' ? a.time - b.time : b.time - a.time);
}
